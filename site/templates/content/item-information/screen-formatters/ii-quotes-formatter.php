<?php
	if (checkformatterifexists($user->loginid, 'ii-quote', false)) {
		$columnindex = 'columns';
		$formatter = json_decode(getformatter($user->loginid, 'ii-quote', false), true);
		$action = 'edit-formatter';
	} else {
		$columnindex = 'columns';
		$formatter = json_decode(file_get_contents($config->paths->content."item-information/screen-formatters/default/ii-quote.json"), true);
	...
	$examplejson = json_decode(file_get_contents($config->paths->content."item-information/screen-formatters/examples/ii-quote.json"), true);
...
<form action="<?php echo $config->pages->ajax."json/ii/ii-quotes-formatter/"; ?>" method="POST" class="screen-formatter-form" id="ii-qt-form">
			...
			<div>
				...
				<div class="tab-content">
					<div role="tabpanel" class="tab-pane active" id="header">
						<?php $table = 'header'; $columns = $headercolumns; include $config->paths->content."item-information/screen-formatters/table.php"; ?>

					</div>
					<div role="tabpanel" class="tab-pane active" id="details">
						<?php $table = 'detail'; $columns = $detailcolumns;  include $config->paths->content."item-information/screen-formatters/table.php"; ?>
					</div>
	...
	<button type="button" class="btn btn-info" onClick="previewtable('#ii-qt-form')"><i class="fa fa-table" aria-hidden="true"></i> Preview Table</button>
