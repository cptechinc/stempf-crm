<?php 
	if (checkformatterifexists($user->loginid, 'vi-payment-history', false)) {
		$columnindex = 'columns';
		$formatter = json_decode(getformatter($user->loginid, 'vi-payment-history', false), true); 
		$action = 'edit-formatter';
	} else {
		$columnindex = 'columns';
		$formatter = json_decode(file_get_contents($config->paths->content."vend-information/screen-formatters/default/vi-payment-history.json"), true); 
...
<form action="<?php echo $config->pages->ajax."json/vi/vi-payment-history-formatter/"; ?>" method="POST" class="screen-formatter-form" id="vi-py-form">
	...
	<button type="button" class="btn btn-info" onClick="previewtable('#vi-py-form')"><i class="fa fa-table" aria-hidden="true"></i> Preview Table</button>
